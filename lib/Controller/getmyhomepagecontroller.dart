...
class GetMyHomepageController extends GetxController {
  //전반적으로 static 붙여있던 것들을 제거. static 사용시 호출한 게 계속 남아있어서 장기적으로 메모리에 좋지 않음.
  var stores = List<Store>().obs; //리스트 만들기, 리스트에도 obs 붙여서 관리할 수 있음, 리스트 불러올 떄는 value로 안불러옴.
  var isLoading = true.obs; //처음에 앱 실행시 로딩중인 상태로 만들기 위해 우선 true로,
